import os
import pandas as pd
import plotly.express as px
import streamlit as st

# Determine the root directory of the project
root_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))

def create_criminal_profiling_dashboard():
    data_file_path = os.path.join(root_dir, 'Component_datasets', 'Criminal_Profiling_cleaned.csv')
    if not os.path.exists(data_file_path):
        st.error("Criminal Profiling dataset missing.")
        return

    Criminal_Profiling = pd.read_csv(data_file_path)

    st.title("Criminal Profiling Dashboard")

    # Age Distribution
    st.subheader("Age Distribution")
    fig_age = px.histogram(Criminal_Profiling, x="age", nbins=20, title="Age Distribution of Criminals")
    st.plotly_chart(fig_age, use_container_width=True)

    # Gender Analysis
    st.subheader("Gender Analysis")
    gender_counts = Criminal_Profiling['Sex'].value_counts()
    fig_gender = px.pie(gender_counts, values=gender_counts.values, names=gender_counts.index, title="Gender Distribution")
    st.plotly_chart(fig_gender, use_container_width=True)

    # Caste Analysis
    st.subheader("Caste Analysis")
    caste_counts = Criminal_Profiling[Criminal_Profiling['Caste'] != 'unknown']['Caste'].value_counts()
    fig_caste = px.bar(x=caste_counts.index[:10], y=caste_counts.values[:10], title="Top 10 Caste Distribution based on Crimes", labels={'x': 'Caste', 'y': 'Count'})
    st.plotly_chart(fig_caste, use_container_width=True)

    # Occupation Analysis
    st.subheader("Occupation Analysis")
    occupation_counts = Criminal_Profiling[(Criminal_Profiling['Occupation'] != "unknown") & (Criminal_Profiling['Occupation'] != "Others PI Specify")]['Occupation'].value_counts()
    occupation_counts = occupation_counts.sort_values(ascending=False)[:10]
    fig_occ = px.bar(x=occupation_counts.values[::-1], y=occupation_counts.index[::-1], orientation='h', title="Top 10 Occupations Associated with Criminal Activities", labels={'x': 'Count', 'y': 'Occupation'})
    st.plotly_chart(fig_occ, use_container_width=True)

    # Top Crime Categories and Sub-Categories
    st.subheader("Top Crime Categories and Sub-Categories")
    top_crime_groups = Criminal_Profiling['Crime_Group1'].value_counts().nlargest(5)
    top_crime_heads = Criminal_Profiling['Crime_Head2'].value_counts().nlargest(5)
    
    tabs = st.tabs(["Category", "Sub-Category"])

    with tabs[0]:
        fig_cat = px.bar(x=top_crime_groups.index, y=top_crime_groups.values, title="Top 5 Most Frequent Crime Group Categories", labels={'x': 'Crime Group', 'y': 'Count'})
        st.plotly_chart(fig_cat, use_container_width=True)

    with tabs[1]:
        fig_sub = px.bar(x=top_crime_heads.index, y=top_crime_heads.values, title="Top 5 Crime Groups Sub-Categories", labels={'x': 'Crime Head', 'y': 'Count'})
        st.plotly_chart(fig_sub, use_container_width=True)
